...
[role="_abstract"]
Developers building images may need to configure authentication and use secrets to handle sensitive data safely within the build process. Access to `build` resources is controlled by Role-Based Access Control, defined by cluster administrators to ensure only authorized users can act securely.
